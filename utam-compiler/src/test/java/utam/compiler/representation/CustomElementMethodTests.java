...
public class CustomElementMethodTests {
  ...
  public void testNestedCustomListWithFilterFindFirst() {
    PageObjectMethod method = new DeserializerUtilities()
        .getContext("filter/customNestedWithFilter").getMethod(METHOD_NAME);
    ...
    expected.addCodeLines(
        "return custom(element, this.test).build(Component.class, elm -> (elm.getText()!= null && elm.getText().contains(arg2)))");
  ...
  public void testCustomListWithSelectorArgsInsideShadow() {
    ...
    PageObjectMethod method = new DeserializerUtilities()
        .getContext("element/customListWithSelectorArg").getMethod(METHOD_NAME);
  ...
  public void testCustomNullableListWithFilter() {
    ...
    expected.addCodeLine("BasicElement root = this.getRootElement()");
    expected.addCodeLine(
        "return custom(root, this.test).buildList(Component.class, elm -> Boolean.TRUE.equals(elm.isVisible()))");
    PageObjectMethod method = new DeserializerUtilities().getContext("filter/customWithFilter")
  ...
  public void testCustomPublicNullableSingleElement() {
    ...
    PageObjectMethod method = new DeserializerUtilities().getContext("element/customPublicNullable")
        .getMethod(METHOD_NAME);
    PageObjectValidationTestHelper.validateMethod(method, expected);
  }
  ...
  public void testCustomNestedFilterFindFirst() {
    TranslationContext context = new DeserializerUtilities()
        .getContext("filter/customFilterNested");
    PageObjectMethod method = context.getMethod(METHOD_NAME);
    ...
    expected.addCodeLines("return custom(scope, this.test.setParameters(selectorArg))"
