...
class UtamRootSelector {
  ...
  UtamRootSelector(
    ...
    isMultipleTypes = Stream.of(css, classchain, uiautomator, accessid).filter(Objects::nonNull).toArray().length > 1;
  }
  ...
  private static void validateUIAutomatorSelector(String parserContext, JsonNode node, String uiautomator) {
    try {
  ...
  private static void validateSubClassChainSelector(String parserContext, JsonNode node, String classchain) {
    ...
    if (classchain.contains("[")) {
      ...
      if (!classchain.startsWith(LocatorClassChain.Quote.SINGLE_BACKTICK.toString())
        ...
        throw new UtamCompilationError(node, VALIDATION.getErrorMessage(1005, parserContext, classchain, getSupportedClassChainQuotes()));
      }
      // validateOperator
      ...
      if ((Stream.of(usedOpers.split(","))
        ...
        throw new UtamCompilationError(node, VALIDATION.getErrorMessage(1006, parserContext, classchain, getSupportedClassChainOperators()));
      }
    }
  }
